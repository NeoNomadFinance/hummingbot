...
class XtExchange(ExchangePyBase):
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        successful_cancellations = None
        failed_cancellations = None
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        for rule in filter(xt_utils.is_exchange_information_valid, trading_pair_rules):
            try:
                ...
                lot_size_filter = next((f for f in filters if f.get("filter") == "QUOTE_QTY"), None)

                min_order_size = Decimal(lot_size_filter.get("min", "1")) if lot_size_filter else Decimal("1")
                tick_size = Decimal("1") / (Decimal("10")**Decimal(rule.get("pricePrecision")))
                step_size = Decimal("1") / (Decimal("10")**Decimal(rule.get("quantityPrecision")))
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                if event_type == "order":
                    ...
                    exchange_order_id = order_update.get("i")
    ...
    async def _all_trade_updates_for_order(self, order: InFlightOrder) -> List[TradeUpdate]:
        ...
        # Do not fetch trades for cached orders(cencelled orders). We are using XT Rest API to process trade fills
        ...
        if order.exchange_order_id is not None and order.client_order_id not in self._order_tracker.cached_orders:
